...
def version():
    print('Version:4.0 by Caesar in 2020/02/20')
...
def read(filename):
        ...
        for record in nmea_file:
            ...
            if(record.sentence_type == 'GGA'):#统计GGA语句条数
                ...
                if(record.gps_qual == 4):
                    ...
                    diff_delay_list.append(float(record.age_gps_data))  #将固定解差分延迟添加至列表
    ...
    if gga_count[0] != 0:
        print('2.GGA total counts:%d, Fixed:%d(%.2f%%), Float:%d(%.2f%%), DGPS:%d(%.2f%%), Single:%d(%.2f%%), NoPos:%d(%.2f%%)' % ( gga_count[0],
                                                                                                                                    gga_count[1],(gga_count[1] / gga_count[0])*100,
                                                                                                                                    gga_count[2],(gga_count[2] / gga_count[0])*100,
                                                                                                                                    gga_count[3],(gga_count[3] / gga_count[0])*100,
                                                                                                                                    gga_count[4],(gga_count[4] / gga_count[0])*100,
                                                                                                                                    gga_count[5],(gga_count[5] / gga_count[0])*100 ))
    else:
    ...
    if gga_count[1] != 0:
        ...
        print('range ±2cm percent:%.2f(%%), range ±1cm percent:%.2f(%%), max-min:%.4f(m)' % ((hgt_ud2cm_count / len(hgts)) * 100,
                                                                                             (hgt_ud1cm_count / len(hgts)) * 100,
                                                                                             max(hgts) - min(hgts)))
    else:
...
#        print('cgcs2000:',cgcs2000_xy) #打印转换后的cgcs2000坐标点集
        ...
        plt.bar(X1, Y1, width=0.5, facecolor='#9999ff', edgecolor='black',label='Solution Percent') #绘制条形码
        plt.legend(loc='upper right')

        solution_text = ['Fix(4)','Float(5)','DGPS(2)','Single(1)','NoPos(0)']

        for x1,y1 in zip(X1,Y1):
            plt.text(x1, y1+0.1, '%.2f(%%)' % float(y1*100.00), ha='center', va= 'top') #在对应条形码上方放置条形码数值

        for i in range(GGAPercentN):            
            plt.text(i, -0.15, solution_text[i], ha='center', va= 'bottom') #在对应条形码下方放置类型
       ...
       # plt.axes([0.025,0.025,0.95,0.95]) #设置坐标轴
        plt.bar(X2, Y2, width=0.3,  facecolor='#ff9999', edgecolor='black',label='Avg Percent') #绘制条形码
        plt.legend(loc='upper right')

        range_text = ['Avg±2cm','Avg±1cm']

        for x2,y2 in zip(X2,Y2):
            plt.text(x2, y2+0.1, '%.2f(%%)' % float(y2*100.00), ha='center', va= 'top') #在对应条形码上方放置条形码数值

        for i in range(HgtPercentN):            
            plt.text(i, -0.15, range_text[i], ha='center', va= 'bottom') #在对应条形码下方放置类型
        ...
        plt.plot(x, miller_x, linewidth=1.0, color="blue", label='E-W(m)', linestyle=':')#绘制longitude实际值趋势
        plt.xlabel('Points')
        ...
        plt.plot(x, miller_y, linewidth=1.0, color="red", label='N-S(m)', linestyle=':')#绘制latitude实际值趋势
        plt.xlabel('Points')
        plt.ylabel('Latitude')
        plt.grid()
        plt.legend()
        ...
        plt.plot(x, hgt_y, linewidth=1.0, color="blue", label='rt hgt', linestyle=':')#绘制实际值趋势
        plt.plot(x, avg_y, linewidth=1.0, color="green", label='avg hgt')#绘制平均值直线
        plt.plot(x, u2cm_y, linewidth=1.0, color="red", label='avg+2cm hgt')#绘制平均值+2cm直线
        plt.plot(x, d2cm_y, linewidth=1.0, color="magenta", label='avg-2cm hgt')#绘制平均值-2cm直线
        plt.xlabel('Points')
        plt.ylabel('Height(m)')
        plt.grid()
        plt.legend()
        ...
        plt.figure(num=3)
        plt.plot(x,sv_list, linewidth=1.0, color="green", linestyle='-')#绘制可用卫星颗数分布图
        plt.xlabel('Points')
        plt.ylabel('SV')
        ...
        plt.figure(num=4)
        plt.plot(x, diff_delay_list, linewidth=1.0, color="red", linestyle='-')#绘制差分延迟分布图
        plt.xlabel('Points')
        ...
        plt.figure(num=6)
        plt.plot(miller_x,miller_y, linewidth=1.0, linestyle=':')#绘制米勒投影XY分布图
        ...
        plt.figure(num=7)
        plt.plot(cgcs2000_x,cgcs2000_y, linewidth=1.0, linestyle=':')#绘制CGCS2000坐标系XY分布图
...
#    filename = 'H:/python study/gps_line.txt'
    version()#打印版本号
    filename = 'H:/gps nmea data/bd982_dual.txt'
    read(filename)
